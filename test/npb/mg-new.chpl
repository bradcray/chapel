...
use BlockDist, Math, StencilDist, Time;
...
const Base: domain(3) dmapped new blockDist({1..nx, 1..ny, 1..nz});
...
record grid {
  ...
  const Inds = (Base by -stride)
                 dmapped new stencilDist(Base,
                                         fluff=(2**stride, 2**stride, 2**stride),
                                         periodic=true);;
  forwarding var Elems: [Inds] real;
  ...
  operator =(ref lhs: grid, rhs: real) {
    lhs.Elems = rhs;
  }
*/
}
...
proc main() {
  var initTimer, benchTimer: stopwatch;

  initTimer.start();
  var V: [Base] real;
  var U, R = [lvl in Levels] new grid(lvl);
  ...
  printResults(rnm2, initTimer.elapsed(), benchTimer.elapsed());
}
...
proc resid(ref R, V, U) {
  const a: coeff = (-8.0/3.0, 0.0, 1.0/6.0, 1.0/12.0);
  const a3d = [(i,j,k) in Stencil] a((i!=0) + (j!=0) + (k!=0));

  const ref UD = U.Inds;
  const      Ustr = U.stride;

  [ijk in UD with (ref R)] R[ijk] = V[ijk] - + reduce [off in Stencil] 
                                 (a3d(off) * U(ijk + Ustr*off));
}


proc psinv(ref U, R) {
  const c: coeff = initCValues(Class);
  const c3d = [(i,j,k) in Stencil] c((i!=0) + (j!=0) + (k!=0));

  const ref RD = R.Inds;
  const     Rstr = R.stride;

  [ijk in RD with (ref U)] U[ijk] += + reduce [off in Stencil] (c3d(off) * R(ijk + Rstr*off));
}


proc rprj3(ref S, R) {
  const w: coeff = (0.5, 0.25, 0.125, 0.0625);
  const w3d = [(i,j,k) in Stencil] w((i!=0) + (j!=0) + (k!=0));

  const ref SD = S.Inds;
  const     Rstr = R.stride;

  [ijk in SD with (ref S)] S[ijk] = + reduce [off in Stencil] (w3d(off) * R(ijk + Rstr*off));
}


proc interp(ref R, S) {
  const IDom: domain(3) = {-1..0, -1..0, -1..0};
  const IStn = [(i,j,k) in IDom] {i..0, j..0, k..0};
  const w = [ijk in IDom] 1.0 / IStn.size;

  const ref SD = S.Inds;
  const     Rstr = R.stride,
            Sstr = S.stride;

  forall ioff in IDom with (ref R) {
    [ijk in SD with (ref R)] R(ijk + Rstr*ioff) 
...
proc initArrays(V, U, R) {
  ...
  zran3(V);
  U(1).Elems = 0.0;  // TODO: Add overloads to support U(1) = 0.0;
}
...
proc zran3(V) {
  ...
  for i in (1..numCharges) {
    (_, pos(i)) = maxloc reduce zip(V, V.indices);
    (_, neg(i)) = minloc reduce zip(V, V.indices);
    V[pos(i)] = 0.5;  // (remove from consideration on next iteration)
    V[neg(i)] = 0.5;
  }
